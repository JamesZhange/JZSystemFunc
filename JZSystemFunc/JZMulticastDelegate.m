/**
 就是 GCDMulticastDelegate，
 无意剽窃，只是每次与XMPPFramework混编时出重定义，太烦，
 改一下名
 James
 **/
...
@interface JZMulticastDelegateNode : NSObject {
@private
...
@interface JZMulticastDelegateEnumerator ()
{
	NSUInteger numNodes;
...
@implementation JZMulticastDelegate
...
- (void)addDelegate:(id)delegate delegateQueue:(dispatch_queue_t)delegateQueue
{
	...
	JZMulticastDelegateNode *node =
	    [[JZMulticastDelegateNode alloc] initWithDelegate:delegate delegateQueue:delegateQueue];
...
- (void)removeDelegate:(id)delegate delegateQueue:(dispatch_queue_t)delegateQueue
{
	...
	{
		JZMulticastDelegateNode *node = delegateNodes[i - 1];
...
{
	for (JZMulticastDelegateNode *node in delegateNodes)
	{
		node.delegate = nil;
		#if __has_feature(objc_arc_weak) && !TARGET_OS_IPHONE
		node.unsafeDelegate = nil;
...
- (JZMulticastDelegateEnumerator *)delegateEnumerator
{
	return [[JZMulticastDelegateEnumerator alloc] initFromDelegateNodes:delegateNodes];
}
...
@implementation JZMulticastDelegateNode
...
@implementation JZMulticastDelegateEnumerator
...
- (BOOL)getNextDelegate:(id *)delPtr delegateQueue:(dispatch_queue_t *)dqPtr
{
	...
	{
		JZMulticastDelegateNode *node = delegateNodes[currentNodeIndex];
		currentNodeIndex++;
